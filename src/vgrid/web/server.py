...
def create_app(
    ...
    *,
    ...
    Args:
        ...
        quote_provider: 实时报价源，默认 ``MootdxSpotProvider``（通达信）。测试 / 离线可注入 stub。
        bar_provider: 历史日线源（关注列表网格适配评分用），默认 ``None`` 走
            ``load_bars`` 的默认源（日线腾讯前复权）。测试 / 离线可注入 stub。
    ...
    app.state.quote_provider = quote_provider or MootdxSpotProvider()

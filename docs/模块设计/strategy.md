...
| 文件 | 内容 |
|---|---|
| `gridlines.py` | `build_levels`（等差/等比）、`bottom_gap`、`shift_window_up`（向上平移）——纯价格数学 |
...
- 向下延伸（跌破下沿逐级放大格距）的实现放在 `Ladder._extend_one`（见下），它要按深度
  放大每格金额、是有状态的增量操作，不在 gridlines 再放一份纯函数，免得两处分叉。

...
- **2026-07-07（review 修复 #9/#17/#18）**：fetch/backtest/scan 三条路径统一 `_guard` 包裹，
  已知异常（配置缺字段/JSON/文件缺失/网络）打一行人话 + 退 1，未预期异常放行 traceback；
  `--top` 钳到 `[0,len]`（负数不再切错切片）；scan 接进度回调，每 25 组往 stderr 打进度。
  补 CLI 错误处理测试（缺配置 / 配置缺字段 / 缺规格，均不打网）。

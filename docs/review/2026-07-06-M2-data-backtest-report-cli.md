...
> 本文件问题编号 1–7 对应总览 #7–#13。

| 本文 # | 总览 # | 状态 | commit |
|---|---|---|---|
| 1 | #7 | ⚖️ 取舍：`_covers` 空洞检测完整修需交易日历，离线无可靠源，暂只文档化限制（`refresh=True` 可绕过） | `d8a73d4` |
| 2 | #8 | ✅ 已解决：补分钟线 akshare（`min_em` / 「时间」列）mock 测试 | `d8a73d4` |
| 3 | #9 | ✅ 已解决：fetch/backtest/scan 统一 `_guard` 错误处理，已知异常打人话 + 退 1 | `d76b554` |
| 4 | #10 | ✅ 已解决：买入持有分母改用 `initial_cash`，与 `total_return` 同口径 | `2b94663` |
| 5 | #11 | ✅ 已解决：年化(365)/夏普(252) 口径不同，报告加脚注 + 模块 docstring 注明 | `2b94663` |
| 6 | #12 | ✅ 已解决：缓存改原子写（`.tmp` + `os.replace`） | `d8a73d4` |
| 7 | #13 | ✅ 已解决：`bars_from_columns` 单次数据内按 ts 去重（后到覆盖前到） | `d8a73d4` |

...
def test_dca_invalid_config_returns_1(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    bad = tmp_path / "bad.json"
...
def test_compare_needs_a_config_returns_1(
    ...
    # 两个配置都不给 → 下载前就退 1（不打网）
    rc = main(["compare", "--symbol", "159920", "--start", "2024-01-02", "--end", "2024-01-03"])
    assert rc == 1
